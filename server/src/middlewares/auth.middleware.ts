...
export const protect = async (req: Request, _res: Response, next: NextFunction) => {
  ...
  if (decoded.divisionId && decoded.role !== "super_admin") {
    ...
    console.log("stillMember:", stillMember, decoded.divisionId);
    if (!stillMember) {
      return next(
  ...
  const activeDivisionId = decoded.divisionId
    ? ((decoded.divisionId as any)?._id?.toString() ?? decoded.divisionId.toString())
    : null;

  if (activeDivisionId && decoded.role !== "super_admin") {
    const stillMember = user.isInDivision(activeDivisionId);
    if (!stillMember) {
      return next(new AppError("Your division access has been revoked, please log in again", 403));
    }
  }

  req.user = user;
  req.activeRole = decoded.role;
  req.activeDivisionId = activeDivisionId;
