...
import { sendSessionCancellationEmail, sendSessionStartEmail } from "../queues/email.queue.js";
...
export const createSession =
  ...
  const bootampId = req.body.bootcamp;
  const bootcamp = await Bootcamp.findById(bootampId);

  if (!bootcamp)
    return next(new AppError("Bootcamp not found", 404, { id: "Not found" }));
