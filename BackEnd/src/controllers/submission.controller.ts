...
export const updateSubmission = catchAsync(
	async (req: Request, res: Response, next: NextFunction) => {
		const { id } = req.params;
		const { githubLink, externalLink, notes } = req.body;
		...
		if (externalLink !== undefined) submission.externalLink = externalLink;
		if (notes !== undefined) (submission as any).notes = notes; 
